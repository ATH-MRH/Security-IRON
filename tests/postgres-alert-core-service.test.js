...
test('PG32A service adds no SQL, parallel savepoints or concurrency primitives',()=>{
 ...
 assert.deepEqual(Object.keys(service).sort(),['init','create','sos','escalateDue','fromIncident','fromBadge','currentUser','config','updateRules','configAudit','notifications','readNotification','list','detail','act','broadcastAlert','receiptAlert','alertReceipts','recipientCandidates'].sort());
});
