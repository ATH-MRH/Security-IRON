...
const AlertCenter = (() => {
  ...
  let rows = [], selected = null, busy = false, timer, detailRequest = 0;
  const e = escapeHtml;
  ...
  async function load() {
    ...
    } catch(err) { error(err); document.getElementById('ac-sync').textContent='Connexion interrompue — données potentiellement anciennes'; }
    finally {busy=false;}
  }
  function renderList() {
  ...
  async function detail(id, redraw=true) {
    ...
    try {
      ...
      if(request !== detailRequest || (!redraw && selected!==id)) return;
      selected=id;
  ...
  async function act(id,action) {
    const buttons=document.querySelectorAll('[data-action]'); buttons.forEach(b=>b.disabled=true);
    try {
      await API.post('/alerts/'+id+'/actions',{action,comment:document.getElementById('ac-comment')?.value||''});
      const input=document.getElementById('ac-comment'); if(input)input.value='';
      await load();
    } catch(err) {error(err);} finally {buttons.forEach(b=>b.disabled=false);}
  }
  function createForm() {
  ...
  async function notifications() {
    try {
      const list=await API.get('/alerts/notifications');
      showModal('Centre de notifications',list.length?list.map(n=>`<button class="ac-alert" data-notification="${n.id}" data-target="${n.alert_id}"><b>${n.read_at?'':'● '}${e(n.message)}</b><small>${date(n.created_at)}</small></button>`).join(''):'<p>Aucune notification.</p>',closeModal,'Fermer');
      document.querySelectorAll('[data-notification]').forEach(b=>b.onclick=async()=>{
        try {await API.post('/alerts/notifications/'+b.dataset.notification+'/read',{});closeModal();navTo('alertes');await detail(b.dataset.target);}catch(err){notify(err.message,'danger');}
      });
  ...
  async function unread() {
    try {
      ...
      if(dot) {dot.style.display=count?'':'none';dot.parentElement.title=count+' notification(s) non lue(s)';}
    } catch { /* The alert center shows connection failures when opened. */ }
  }
  function start() {
    clearInterval(timer);
    unread();
    timer=setInterval(()=>{if(document.hidden)return;unread();if(document.getElementById('page-alertes').classList.contains('active'))load();},5000);
    document.getElementById('ac-rules').hidden=!isAdmin();
  }
  return {load,renderList,createForm,rules,notifications,start};
})();
